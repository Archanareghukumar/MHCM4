    ...
    def execute(self):
        ...
        for subject in subjects:
            try:  
                ...
                for cols in ['MHENDAT', 'MHTERM']:
                    mh_df = mh_df[mh_df[cols].notna()]
                for cols in ['CMONGO', 'CMINDC', 'CMENDAT']:
                    cm_df = cm_df[cm_df[cols].notna()]
                for cols in ['DSSTDAT']:
                    ds_df = ds_df[ds_df[cols].notna()]
                           
                if 'CMDU' in cm_df.columns.tolist():
                    cm_df = cm_df[cm_df['CMDU'].notna()]
                if(cm_df['CMSTDAT'] < ds_df['DSSTDAT']) &\
                    mh_df['MHTERM']==cm_df['CMINDC'] &\
                    mh_df['MHENDAT'] < cm_df['CMENDAT']    
          
                for ind in range(mh_df.shape[0]):
                    #try:
                        mh_record = mh_df.iloc[[ind]]
                        mhterm = mh_record['MHTERM'].values[0]
                        mhendat = mh_record['MHENDAT'].apply(utils.get_date)
                        ...
                        if (match_flag is True):
                            ...
                            for cm_ind in range(cm_match_df.shape[0]):
                                ...
                                cmendat = cm_record['CMENDAT'].apply(utils.get_date)
                                try:
                                ...
                                cmindc = cm_record['CMINDC'].values[0]
                                cmtrt = cm_record['CMTRT'].values[0]
                                cmdu = cm_record['CMDU'].values[0] if 'CMDU' in cm_record.columns.tolist() else ''
                                ...
                                if query_flag is True: 
                                            ...
                                            mhterm = mh_record['MHTERM'].values[0]
                                            cmtrt = cm_record['CMTRT'].values[0]
                                            mh_endat = mh_record['MHENDAT'].values[0]
                                            cm_endat = cm_record['CMENDAT'].values[0]
...
if __name__ == '__main__':
    ...
    rule_id = sys.argv[2]
    version = sys.argv[3]
    rule = MHCM4(study_id, rule_id, version)
    rule.run()